...
init([]) ->	
	...
	{ok, SLASH} = re:compile("/.*", Options),
	{ok, QUERY_STRING} = re:compile("\\?.*", Options),
	SecondLevelDomains = sets:from_list(?SECOND_LEVEL_DOMAINS),%dobrze znane domeny drugiego poziomu
	State = #state{http_pattern = HTTP, www_pattern = WWW, slash_pattern = SLASH, query_string_pattern = QUERY_STRING, second_level_domains = SecondLevelDomains},
	{ok, State}.
	...
	Reply = case length(Reversed) of
		...
		N ->
			...
			case sets:is_element(TwoLevels, SecondLevelDomains) of
				true ->
					Third ++ "." ++ TwoLevels;%jezeli domena drugiego poziomu jest dobrze znana, to zwracamy trzy czlony
				false ->
					TwoLevels
			end			
	end,
